...
def main(spec: str):
    with open("metadata.csv", "w", newline="") as csvfile:
        ...
        for testcase in glob.glob("RML*"):
            ...
            input_found = False
            for pattern, mime_type in INPUT_PATTERN_MIME_PAIRS:
                glob_path = os.path.join(testcase, pattern)
                found_paths = glob.glob(glob_path)
                if len(found_paths) > 0:
                    path = found_paths[0]
                    input1 = f"{os.path.basename(path)}"
                    input_format1 = mime_type
                    input_found = True

            if not input_found:
                print(f"WARNING: Input file is missing for testcase: {testcase}!")
            ...
            for index, i in enumerate([input1, input2, input3]):
                ...
                if "http://w3id.org/rml/resources" in i:
                    ...
                    try:
                        ...
                        try:
                            with open(
                                os.path.join(testcase, i), encoding="utf-16"
                            ) as f:
                                input_html = (
                                    f"**Input{inputCount}**\n```\n{f.read()}\n```\n\n"
                                )
